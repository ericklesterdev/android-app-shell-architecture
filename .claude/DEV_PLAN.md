...
16 commits across 7 phases. Each step compiles independently and tells a natural development story.

Showcase apps and mock JSON are set up first in each feature phase — devs get a runnable entry point before building screens.
...
## Phase 2: Platform Implementations (Steps 3–5)
...
## Phase 3: Teams Feature (Steps 6–8) — Manual DI

### Step 6 — Scaffold feature/teams with showcase app, mock data, and data layer
...
### Step 8 — Team detail screen with roster and favorite FAB
...
## Phase 4: Scores Feature (Steps 9–10) — Dagger 2

### Step 9 — Scaffold feature/scores with showcase app, mock data, data layer, and Dagger setup
...
## Phase 5: App Shell (Steps 11–13)

### Step 11 — App Shell scaffold
- `app/` module — Android application in root project
- `AppConfig` — `useMockNetwork`, `apiKey`, `baseUrl`
- `AppContainer` — composition root creating platform instances + feature instances
- `NbaHubApplication` — holds `AppContainer`
- `MainActivity` — Scaffold, bottom nav (Scores + Teams), `NavHost` with 3 routes
- Cross-feature navigation: Scores `onTeamClick` → Teams detail
- Theme toggle icon in top bar
- Material 3 theme (`Theme.kt`, `Color.kt`, `Type.kt`)
- Copy mock JSON to `app/src/main/assets/mock/` for full-app mock mode
- **Commit:** `feat: add App Shell with bottom navigation, theme toggle, and dependency wiring`
- **Verify:** `./gradlew :app:assembleDebug` — run full app, both tabs work

### Step 12 — Polish theme + loading/error states
...
### Step 13 — Dark mode + persistent theme preference
...
## Phase 6: SDK (Steps 14–15)

### Step 14 — NBA Scores SDK
...
| 3 | `feat: add platform/network-mock for asset-based JSON responses` | Platform |
| 4 | `feat: add platform/network with OkHttp NetworkClient implementation` | Platform |
| 5 | `feat: add platform/storage with DataStore-backed favorites persistence` | Platform |
| 6 | `feat: scaffold feature/teams with showcase app, mock data, and service layer` | Teams |
| 7 | `feat: add Teams list screen with conference filters and favorites` | Teams |
| 8 | `feat: add Team detail screen with roster and favorite toggle FAB` | Teams |
| 9 | `feat: scaffold feature/scores with showcase app, mock data, and Dagger 2 DI` | Scores |
| 10 | `feat: add Scores list screen with game cards, status badges, and favorites` | Scores |
| 11 | `feat: add App Shell with bottom navigation, theme toggle, and dependency wiring` | App Shell |
| 12 | `feat: polish theme, team colors, loading states, and navigation transitions` | App Shell |
| 13 | `feat: add dark mode with persistent theme preference` | App Shell |
| 14 | `feat: add nba-scores-sdk with embeddable live scores widget` | SDK |
| 15 | `feat: add SDK showcase app simulating external company integration` | SDK |
| 16 | `docs: add comprehensive README with architecture docs and setup instructions` | Docs |
